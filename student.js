...
$("#student-load").addEventListener("click",async()=>{
  const id=$("#student-id").value.trim();if(!id)return;const st=$("#student-api-status");
  try{
    const w=await GPSP_API.studentWorkspace(id);const s=w.student||{};
    ...
    $("#s-reviews").innerHTML=revs.length?revs.slice().reverse().map(r=>`<div class="timeline-item"><div>${esc(r.part||"عام")}</div><div>${esc(r.feedback||r.majorNotes||r.minorNotes||"—")}</div><span class="badge blue">${esc(r.score||"—")}</span></div>`).join(""):'<div class="empty">لا توجد تغذية راجعة.</div>';
    st.textContent="متصل";st.className="badge green";
  }catch(e){st.textContent=e.message==="API_URL_NOT_CONFIGURED"?"API غير مربوط":"تعذر الاتصال";st.className="badge orange";}
});
